...
class JacksonCodecTest {
    ...
    <T> void canDecodeSubTypesOfIncomingMessages(
            String json, Class<T> expectedParsedType, Consumer<T> assertionsOnResult)
            throws Exception {
    ...
    static Stream<Arguments> canDecodeSubTypesOfIncomingMessages() {
        return Stream.of(
                arguments(
                        "{\"txId\":\"<random-incoming-id>\", \"messageType\":\"chat\","
                                + " \"content\":\"HELLO\"}",
                        IncomingChatMessage.class,
                        (Consumer<IncomingChatMessage>)
                                m -> assertThat(m.content()).isEqualTo("HELLO")),
                arguments(
                        "{\"txId\":\"<random-incoming-id>\", \"messageType\":\"game\", \"action\":"
                                + " {\"type\":\"play\", \"cards\": [\"star_ace\", \"sword_ace\"]}}",
                        GameActionMessage.class,
                        (Consumer<GameActionMessage>)
                                m -> assertThat(m.action()).isEqualTo(sampleGameParam())));
    }

    @ParameterizedTest
    @MethodSource
    void canEncodeSubtypesOfOutgoingMessages(OutgoingMessage toEncode, String expectedJson)
            throws Exception {
    ...
    static Stream<Arguments> canEncodeSubtypesOfOutgoingMessages() {
        ...
        return Stream.of(
                arguments(
                        new OutgoingChatMessage(UserId.of("dummy"), "hello", "<random-id>"),
                        "{messageType:'chat', from:'dummy', content: 'hello', txId:"
                                + " '<random-id>'}"),
                arguments(
    ...
    void jacksonMappingIsSymetric() throws Exception {
        ...
        final GameActionMessage initialObj =
                new GameActionMessage("<random-incoming-id>", sampleGameParam());
