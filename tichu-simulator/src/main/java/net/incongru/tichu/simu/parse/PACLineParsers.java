...
class PACLineParsers extends AbstractLineParsers<Simulation.PostActionCommand> {
    ...
    PACLineParsers(PostActionCommandFactory pacFactory) {
        super(
                "post-action-command",
                Arrays.asList(
                        simpleParser(t -> expect(t, "success"), _ -> pacFactory.expectSuccess()),
                        simpleParser(
                                t -> expect(t, "error"), // TODO exactly what are our scenarios, if
                                // not invalid plays
                                t -> pacFactory.expectError(t.remainder())),
                        simpleParser(
                                t -> expect(t, "invalid-play"), // TODO other keywords?
                                t -> {
                                    final PostActionCommandFactory.ExpectablePlayResult
                                            expectedPlayResult =
                                                    NameableEnum.byName(
                        ...
                        simpleParser(
                                t -> expect(t, "game"),
                                t -> {
                                    final PostActionCommandFactory.ExpectableGameState
                                            expectedGameState =
                                                    NameableEnum.byName(
                                                            PostActionCommandFactory
