        ...
        super(
                ...
                Arrays.asList(
                        simpleParser(
                                t -> t.test(0, "init"),
                                _ ->
                                        InitialiseGameParam.withActor(
                                                UserId.of("dummy"))), // TODO player id ?
                        simpleParser(
                                t -> t.test(1, "joins") && t.test(1, "team"), // or "table"
                                t -> {
                                    final String playerName = t.pop(0);
                                    final int team =
                                            t.popInt(0) - 1; // team is 0-indexed, but we expect the
                                    // text interface to be 1-indexed
                                ...
                                t -> {
                                    ...
                                    if (t.count() < 1) {
                                        throw new LineParserException(
                                                t,
                                                "No cards played, use the '<player> passes' action"
                                                        + " instead.");
                                    }
                                    final Set<Card> cards = t.remainderAsCards();
                        ...
                        simpleParser(
                                ...
                                t -> {
                                    final String playerName = t.pop(0);
                                    return PlayerPlaysParam.withActor(
                                            UserId.of(playerName), Collections.emptySet());
                                }),
                        simpleParser(
