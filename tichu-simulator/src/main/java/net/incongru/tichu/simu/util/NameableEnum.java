...
public interface NameableEnum {
    ...
    static <E extends Enum<E> & NameableEnum> E byName(
        Class<E> enumClass,
        String name
    ) {
        for (E p : enumClass.getEnumConstants()) {
            ...
            if (
                Lists.transform(p.altNames(), String::toLowerCase).contains(
                    name.toLowerCase()
                )
            ) {
                return p;
    ...
    static <E extends Enum<E> & NameableEnum> String allNamesOf(
        Class<E> enumClass
    ) {
