...
class CardTest {
    ...
    void suitComparatorDoesNotMessUpSpecialCards() {
        ...
        Assertions.assertThat(FluentIterable.from(new CardDeck().allRemaining()).toSortedList(BY_SUIT)).hasSize(56);
    ...
    void predicates() {
        ...
        assertThat(CardPredicates.is(CardSpecials.Dog)).rejects(Jade_Queen, MahJong, Dragon, Phoenix);

        assertThat(CardPredicates.is(CardNumbers.Queen, CardSuit.Jade)).accepts(Jade_Queen);
        assertThat(CardPredicates.is(CardNumbers.Queen, CardSuit.Jade)).rejects(Sword_7, Dog, MahJong, Dragon, Phoenix);
