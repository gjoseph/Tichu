...
public class TichuRules {
    ...
    // If rules were to change, one would not only subclass TichuRules, but probably provide
    // different factories too.
    ...
    public Player whoStarts(Players players) {
        return players.stream()
                .filter(hasMahjong)
                .findFirst()
                .orElseThrow(() -> new IllegalStateException("No player has the Mahjong!?"));
    }
    ...
    public boolean isValid(Play play) {
        ...
        // TODO is this needed? dont we trust the Play.Factories? Should Play have a isValid()
        // method?
