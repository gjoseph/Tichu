...
class TerminalHandler implements TichuWebSocketHandler {
    ...
    private ask = <T, K extends PromptKey>(
        type: K,
        // Parameters<PromptTypes[K]>[0] gets the exact Config type for that specific prompt
        question: Parameters<PromptTypes[K]>[0]
    ) => { //: Promise<T> => {
        this.closeCurrentPrompt();
...
type CheckboxConfig = Parameters<typeof prompts.checkbox>[0];
type SelectConfig = Parameters<typeof prompts.select>[0];

type DynamicQuestion =

    | ({ type: 'input' } & InputConfig)
    | ({ type: 'checkbox' } & CheckboxConfig)
