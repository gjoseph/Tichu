...
class DealCardsTest {
    ...
    void dealsNCardsToEachPlayer() {
        ...
        new DealCards()
            .exec(ctx, DealCardsParam.withActor(UserId.of("jules"), 8));

        assertThat(ctx.player(UserId.of("alex")).hand()).hasSize(8);
        assertThat(ctx.player(UserId.of("charlie")).hand()).hasSize(8);
        assertThat(ctx.player(UserId.of("jules")).hand()).hasSize(8);
        assertThat(ctx.player(UserId.of("quinn")).hand()).hasSize(8);
    }
}
